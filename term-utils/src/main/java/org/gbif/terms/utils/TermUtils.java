...
public class TermUtils {
  ...
  private static final Set<DwcTerm> DwC_EXTENSION_EXCLUSIONS =
          Set.copyOf(
                  Stream.concat(
                          DwcTerm.listByGroup(DwcTerm.GROUP_MEASUREMENTORFACT).stream(),
                          DwcTerm.listByGroup(DwcTerm.GROUP_RESOURCERELATIONSHIP).stream()
   ...
   * (Needed for the next declaration.)
   */
  private static final Set<GbifTerm> GBIF_PROPERTIES_EXCLUSIONS = Set.of(
   ...
   */
  private static final Map<Term,String> TERMS_IDENTICAL_AFTER_INTERPRETATION = Map.of(
      DwcTerm.geodeticDatum, Occurrence.GEO_DATUM);
   ...
   */
  private static final Set<Term> TAXON_TERMS_POPULATED_BY_INTERPRETATION = Set.of(
    DwcTerm.kingdom,
    DwcTerm.phylum,
    DwcTerm.class_,
    DwcTerm.order,
   ...
   * <p>These are all explicit Java properties on the {@link org.gbif.api.model.occurrence.Occurrence} class.
   */
  private static final Set<Term> TERMS_POPULATED_BY_INTERPRETATION = Set.of(
   ...
   * verbatim record
   */
  private static final Set<Term> TERMS_SUBJECT_TO_INTERPRETATION = Stream.<Stream<Term>>of(
  ...
  public static List<Term> verbatimTerms() {
    return Stream.<Stream<? extends Term>>of(
            Stream.of(GbifTerm.gbifID),
            DwC_DC_PROPERTIES.stream(),
            DwC_PROPERTIES.stream()
