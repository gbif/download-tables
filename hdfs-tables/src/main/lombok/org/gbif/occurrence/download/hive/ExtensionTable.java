...
public class ExtensionTable {
  ...
  public static List<ExtensionTable> tableExtensions() {
    return EXTENSION_TABLES.keySet().stream().map(ExtensionTable::new).collect(Collectors.toList());
  }
  ...
  public Set<String> getInterpretedFields() {
    ...
    interpretedFields.addAll(schema.getFields().stream().map(f -> hiveColumnName(f.name())).filter(fieldName -> !fieldName.startsWith("v_") && !fieldName.equalsIgnoreCase(GBIFID_FIELD) && !fieldName.equalsIgnoreCase(DATASET_KEY_FIELD)).toList());
  ...
  public List<Term> getInterpretedFieldsAsTerms() {
    ...
    interpretedFields.addAll(schema.getFields().stream().filter(field -> !field.name().startsWith("v_") && !field.name().equalsIgnoreCase(GBIFID_FIELD) && !field.name().equalsIgnoreCase(DATASET_KEY_FIELD)).map(f -> TERM_FACTORY.findPropertyTerm(f.doc())).toList());
    return interpretedFields;
  }
  ...
  public Set<String> getVerbatimFields() {
    ...
    verbatimFields.addAll(schema.getFields().stream().map(f -> hiveColumnName(f.name())).filter(fieldName -> fieldName.startsWith("v_") || fieldName.equalsIgnoreCase(GBIFID_FIELD) || fieldName.equalsIgnoreCase(DATASET_KEY_FIELD)).toList());
  ...
  public java.lang.String toString() {
    return "ExtensionTable(leafNamespace=" + this.leafNamespace + ", schema=" + this.getSchema() + ", extension=" + this.getExtension() + ", term=" + this.getTerm() + ")";
  }
}
