 ...
 * maintenance.
 */
public final class OccurrenceHDFSTableDefinition {
  public static void main(String[] args) {
    System.out.println("CREATE TABLE IF NOT EXISTS occurrence (\n" + OccurrenceHDFSTableDefinition.definition().stream().map(field -> field.getHiveField() + " " + field.getHiveDataType()).collect(Collectors.joining(", \n")) + ") STORED AS PARQUET TBLPROPERTIES (\"parquet.compression\"=\"SNAPPY\")");
  }

  private static final Set<Term> ARRAYS_FROM_VERBATIM_VALUES = ImmutableSet.of(DwcTerm.recordedByID, DwcTerm.identifiedByID, DwcTerm.datasetID, DwcTerm.datasetName, DwcTerm.recordedBy, DwcTerm.identifiedBy, DwcTerm.otherCatalogNumbers, DwcTerm.preparations, DwcTerm.samplingProtocol, DwcTerm.higherGeography, DwcTerm.georeferencedBy, DwcTerm.associatedSequences);
  ...
  private static List<InitializableField> interpretedFields() {
    ...
    Map<Term, String> initializers = ImmutableMap.<Term, String>builder().put(GbifTerm.datasetKey, columnFor(GbifTerm.datasetKey)).put(GbifTerm.protocol, columnFor(GbifTerm.protocol)).put(GbifTerm.publishingCountry, columnFor(GbifTerm.publishingCountry)).put(DwcTerm.eventType, columnFor(DwcTerm.eventType)).put(IucnTerm.iucnRedListCategory, columnFor(IucnTerm.iucnRedListCategory)).put(GbifInternalTerm.classifications, columnFor(GbifInternalTerm.classifications)).put(GbifInternalTerm.taxonomicStatuses, columnFor(GbifInternalTerm.taxonomicStatuses)).put(GbifInternalTerm.classificationDetails, columnFor(GbifInternalTerm.classificationDetails)).put(GbifTerm.checklistKey, columnFor(GbifTerm.checklistKey)).build();
    ImmutableList.Builder<InitializableField> builder = ImmutableList.builder();
  ...
  private static List<InitializableField> internalFields() {
    Map<Term, String> initializers = new ImmutableMap.Builder<Term, String>().put(GbifInternalTerm.publishingOrgKey, columnFor(GbifInternalTerm.publishingOrgKey)).put(GbifInternalTerm.installationKey, columnFor(GbifInternalTerm.installationKey)).put(GbifInternalTerm.institutionKey, columnFor(GbifInternalTerm.institutionKey)).put(GbifInternalTerm.collectionKey, columnFor(GbifInternalTerm.collectionKey)).put(GbifTerm.projectId, columnFor(GbifTerm.projectId)).put(GbifInternalTerm.programmeAcronym, columnFor(GbifInternalTerm.programmeAcronym)).put(GbifInternalTerm.hostingOrganizationKey, columnFor(GbifInternalTerm.hostingOrganizationKey)).put(GbifInternalTerm.isInCluster, columnFor(GbifInternalTerm.isInCluster)).put(GbifInternalTerm.dwcaExtension, columnFor(GbifInternalTerm.dwcaExtension)).put(GbifInternalTerm.eventDateGte, columnFor(GbifInternalTerm.eventDateGte)).put(GbifInternalTerm.eventDateLte, columnFor(GbifInternalTerm.eventDateLte)).build();
    ImmutableList.Builder<InitializableField> builder = ImmutableList.builder();
  ...
  private static List<InitializableField> extensions() {
    ...
    for (Extension e : extensions) {
      builder.add(new InitializableField(extensionTerm(e), columnFor(e), HiveDataTypes.TYPE_STRING));
  ...
  public static List<InitializableField> definition() {
    return ImmutableList.<InitializableField>builder().add(keyField()).addAll(verbatimFields()).addAll(internalFields()).addAll(interpretedFields()).addAll(extensions()).build();
  }
  ...
  private static InitializableField keyField() {
    return new InitializableField(GbifTerm.gbifID, GbifTerm.gbifID.simpleName().toLowerCase(Locale.ENGLISH), HiveDataTypes.typeForTerm(GbifTerm.gbifID, true));
    // verbatim context
  }
  ...
  private static InitializableField interpretedField(Term term) {
    ...
    if (HiveDataTypes.TYPE_ARRAY_STRING.equals(HiveDataTypes.typeForTerm(term, false)) && ARRAYS_FROM_VERBATIM_VALUES.contains(term)) {
  ...
  private static InitializableField interpretedField(Term term, String initializer) {
    return new InitializableField(term, term.simpleName().toLowerCase(Locale.ENGLISH), 
