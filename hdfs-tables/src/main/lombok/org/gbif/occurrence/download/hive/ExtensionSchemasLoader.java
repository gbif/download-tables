...
public class ExtensionSchemasLoader {
  private static String normalizeFileName(String name) {
    String result = Arrays.stream(name.split("_")).map(String::toLowerCase).collect(Collectors.joining("-"));
  ...
  private static Extension parseExtension(String name) {
    return Extension.valueOf(Arrays.stream(name.replace("Table", "").split("(?=[A-Z])")).map(String::toUpperCase).collect(Collectors.joining("_")));
  }
